...
fn register_shipment_without_products() {
    new_test_ext().execute_with(|| {
        ...
        let result = PalletTracking::register_shipment(
        ...
        assert_eq!(
            PalletTracking::shipment_by_id(&id),
            Some(Shipment {
                id: id.clone(),
                owner: owner,
...
fn register_shipment_with_invalid_sender() {
    new_test_ext().execute_with(|| {
        assert_noop!(
            PalletTracking::register_shipment(
                Origin::none(),
...
fn track_shipment_pickup() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(PalletTracking::track_shipment(
            Origin::signed(owner),
            shipment_id.clone(),
            ShippingOperation::Pickup,
            now,
        ...
        assert_eq!(
            PalletTracking::shipment_by_id(&shipment_id),
            Some(Shipment {
