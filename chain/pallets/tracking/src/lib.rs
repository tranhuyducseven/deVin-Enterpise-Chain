...
decl_storage! {
    trait Store for Module<T: Trait> as PalletTracking {
        // Shipments
    ...
    pub struct Module<T: Trait> for enum Call where origin: T::Origin {
        ...
        fn offchain_worker(block_number: T::BlockNumber) {
            ...
            let mut lock = StorageLock::<Time>::with_deadline(
                b"pallet_tracking_ocw::lock",
                rt_offchain::Duration::from_millis(LOCK_TIMEOUT_EXPIRATION)
            );

            match lock.try_lock() {
                Ok(_guard) => { Self::process_ocw_notifications(block_number); }
                Err(_err) => { debug::info!("[pallet_tracking_ocw] lock is already acquired"); }
            };
        }
    }
}
...
impl<T: Trait> Module<T> {
    ...
    fn process_ocw_notifications(block_number: T::BlockNumber) {
        ...
        let last_processed_block_ref =
            StorageValueRef::persistent(b"pallet_tracking_ocw::last_proccessed_block");
        let mut last_processed_block: u32 = match last_processed_block_ref.get::<T::BlockNumber>() {
            Some(Some(last_proccessed_block)) if last_proccessed_block >= block_number => {
                debug::info!(
                    "[pallet_tracking_ocw] Skipping: Block {:?} has already been processed.",
                    block_number
                );
                return;
            }
            ...
            _ => {
                debug::error!("[pallet_tracking_ocw] Error reading pallet_tracking_ocw::last_proccessed_block.");
                return;
            }
        };
        ...
        for current_block in start_block..end_block {
            debug::debug!(
                "[pallet_tracking_ocw] Processing notifications for block {}",
                current_block
            );
            ...
            if let Err(err) = listener_results {
                debug::warn!("[pallet_tracking_ocw] notify_listener error: {}", err);
                break;
            }
        ...
        if last_processed_block >= start_block {
            ...
            debug::info!(
                "[pallet_tracking_ocw] Notifications successfully processed up to block {}",
